...
## v2 — Few-shot Prompt 增强（exp-v2-prompt）
...
```python
# 新增 FEWSHOT_PROMPT：
# - 保留 v1 简洁的「看视频→输出JSON」结构
# - 加入1个标注示例（爆装刺激视频的场景描述→JSON）
# - 各字段加中文释义（消除歧义）
# - 新增 --use_cot 和 --max_tokens 参数
```

### 踩坑

- CoT 版本（逐步推理 prompt）导致模型输出 markdown 代码块 + 标签混乱，得分暴跌到 29.95
- 回退到简化的 Few-shot 格式后恢复正常

### 对比结果（训练集前 10 条）

| 字段 | v1 Scene Detect | v2 Few-shot | Δ |
|------|----------------|-------------|------|
| visual_source_type | 0.9000 | 0.8000 | -0.10 |
| has_real_person | 1.0000 | 0.9000 | -0.10 |
| narrative_structure | 0.2000 | **0.3000** | +0.10 |
| selling_point | 0.7000 | 0.6000 | -0.10 |
| cta_type | 0.4000 | **0.7000** | +0.30 |
| claim_type | 0.5217 | **0.6957** | +0.17 |
| core_action | 0.6286 | **0.7027** | +0.07 |
| growth_payoff | 0.2857 | **0.4615** | +0.18 |
| **Overall Avg** | **0.5795** | **0.6450** | **+0.07** |
| **预估得分** | **40.57** | **45.15** | **+4.58** |

### 分析

- ✅ **cta_type (+0.30)** 提升最大 — Few-shot 示例教会了模型识别结尾 CTA
- ✅ **growth_payoff (+0.18)** — 示例中「战力大幅提升+装备获得」引导了成长收益判断
- ✅ **claim_type (+0.17)** — 利益承诺识别更准
- ✅ **narrative_structure (+0.10)** — 从 0.2→0.3，但仍偏低
- ⚠️ visual_source_type/has_real_person/selling_point 小幅下降 — 示例偏「实机+爆装」可能有偏差
...
|------|---------|-----|
| v0 Baseline | 36.31 | — |
| v1 Scene Detect | 40.57 | +4.26 |
| v2 Few-shot | **45.15** | **+4.58** |
| **总计** | — | **+8.84** |
